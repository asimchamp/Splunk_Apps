...
MAX_POP_STAZAS = 50             # keep 50 most popular stanzas
MAX_SECS_BEFORE_NO_DEDUP = 10   # if previewing takes more than 10 seconds, stop trying to dedup sourcetypes
...
##        return s
    ...
    def keepPopular(self, stanzas):
        ...
        popular_names = self.popularStanzaNames(stanzas)[:MAX_POP_STAZAS]
        for name in stanzas.keys():
            ...
            try:
                ...
                for attr, value in values.items():
                    ...
                    if attr in ['TIME_PREFIX', 'LINE_BREAKER', 'BREAK_ONLY_BEFORE', 'MUST_BREAK_AFTER', 'MUST_NOT_BREAK_BEFORE', 'MUST_NOT_BREAK_AFTER'] and len(value)>3:
                        if not re.search("(?ms)" + value, data):
                            bad = True
...
##                            break
    ...
    def dedupStanzas(self, stanzas, eventdata):
        ...
        for name in names:
            try:
                # stop analyzing after we have N good different parses
                if len(seenParses) > MAX_PARSE_BEFORE_GIVE_UP:
                    stanzas[name]['count'] = -1
                    continue
                
                now = time.time()
                events = self.getPreviewEvents(eventdata, name, stanzas.get(name, {}))
                ...
                try:
                    ...
                    if math.abs(timediff) > 10*60:
                        stanzas[name]['count'] += 10000
                except:
                    pass
                ...
                if parseID in seenParses:
                    stanzas[name]['count'] = -1
                    #del stanzas[name]
                    print "DELETED DUP PARSE:", name
                seenParses.add(parseID)
            except Exception, e:
                self.addMsg('warn', 'Problem deduping %s: %s' % (name, e))
                self.addMsg('warn', 'Stacktrace: %s' % traceback.format_exc())
    ...
    def getPreviewEvents(self, eventdata, stanza, attrs):
        ...
        try:
            ...
            kwargs = { 'input.path':filename, 'output_mode':'json' } #, 'props.sourcetype':'asdfsadfsdf'} # give dummy sourcetype hoping that prevents classifying data
