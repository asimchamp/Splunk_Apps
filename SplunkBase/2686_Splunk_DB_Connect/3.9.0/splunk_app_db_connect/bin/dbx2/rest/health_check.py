...
class HealthChecker(SimpleRest):
    ...
    def _check_java_server_configuration_health(self):
        ...
        input_url = "{}://{}:{}/services/data/modular-inputs/server".format(self.splunk_scheme, self.splunk_host,
                                                                            self.splunk_port)
        logger.debug("_check_java_server_configuration_health url is {}".format(input_url))
        try:
    ...
    def _forward_request_to_proxy(self):
        ...
        forward_url = "{}://{}:{}/servicesNS/{}/splunk_app_db_connect/{}/dbxproxy/{}" \
            .format(self.splunk_scheme, self.splunk_host, self.splunk_port, self.userName, self.pathParts[1],
    ...
    def _check_java_server_availability(self):
        taskserver_status_url = "{}://{}:{}/servicesNS/{}/splunk_app_db_connect/db_connect/dbxproxy/taskserver" \
            .format(self.splunk_scheme, self.splunk_host, self.splunk_port, self.userName)
        logger.debug("_check_java_server_availability url is {}".format(taskserver_status_url))
        status = Status.OK
        detail = ''
        try:
