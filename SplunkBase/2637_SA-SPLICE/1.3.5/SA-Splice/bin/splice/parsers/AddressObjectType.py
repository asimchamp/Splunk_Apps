...
class AddressObjectType(BaseObjectType):
	...
	if pattern_type == "Regex" :
		for v in value_list:
			ret.append( splindicator.SPLIndicator(self.object_id, 'regex', 'ipv4-cidr', v) )
	...
	if pattern_type == None :
		...
		if condition == "InclusiveBetween" :
			...
			ret.append( splindicator.SPLIndicator(self.object_id, 'value', 'ipv4-range', "%s-%s" % (low_min, high_max)) )

		# CIDR List
		else:
			for v in value_list:
				if iptools.ipv4.validate_cidr( v ) :
					ret.append( splindicator.SPLIndicator(self.object_id, 'value', 'ipv4-cidr', v) )
				else:
	...
	"""
	Known formats:
	{'address_value': {'value': '@state.gov', 'condition': 'Contains'}, 'xsi:type': 'AddressObjectType', 'category': 'e-mail'}
	{'address_value': jdoe@state.gov', 'xsi:type': 'AddressObjectType', 'category': 'e-mail'}
	"""
	...
	ret = []
	if isinstance(address_value, basestring) :
		ret.append( splindicator.SPLIndicator(self.object_id, 'value', 'email', address_value) )
	elif isinstance(address_value, dict ):
		value     = common.get_value_from_dict(address_value, 'value')
		condition = common.get_value_from_dict(address_value, 'condition', 'Equals')

		# if not 'Equals' we assume a regex pattern.
		pattern_type = 'value'
		if condition != 'Equals' :
			pattern_type = 'regex'
		ret.append( splindicator.SPLIndicator(self.object_id, pattern_type, 'email', value) )
	else:
		self.log('ERROR', 'AddressObjectType(category=email) unknown type of %s' % type(address_value))
