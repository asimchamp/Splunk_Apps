...
def server_extractor_from_xml(xml_str, logger):
    ...
    try:
        root = etree.fromstring(  # nosemgrep false-positive : The 'xml_str' is the arg which is fetched from the stdin while invoking the modular input. It doesn't take any user/external inputs.
            xml_str
        )
        ...
        if os.path.isfile(config_file_path):
            server_name = (
                etree.parse(  # nosemgrep false-positive : The 'config_file_path' passed in parse() is the arg which is fetched from the stdin while invoking the modular input. It doesn't take any user/external inputs.
                    config_file_path
                )
                .find(".//jmxserver")
                .get("jmxaccount", "")
...
def input_extractor_from_xml(xml_str, logger):
    ...
    try:
        root = etree.fromstring(xml_str) # nosemgrep false-positive : The 'xml_str' is the arg which is fetched from the stdin while invoking the modular input. It doesn't take any user/external inputs.
