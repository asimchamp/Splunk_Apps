    ...
    function(ChartView,SearchManager){
        ...
        ChartView.on("click", function (e) {
            ...
            var search_query = new SearchManager({
                search: 'eventtype="cisco_mso" mso_api_endpoint=siteHealth mso_host='+mso_host+" mso_site_id= "+mso_site_id+ ' | rex field=urls "https://(?<url>.*)"$ | mvexpand url | where name = "'+site_name+'" | table url, platform',
            ...
            search_query.on('search:done', function(properties) {
                ...
                search_query_result.on("data", function() {
                    ...
                    if(apic_platform == "on-premise")
                        window.open('/app/cisco-app-ACI/fabric_dashboard?form.mso_host='+mso_host+'&form.apic_host='+apic_host+'&form.time_token.earliest='+time_token_earliest+'&form.time_token.latest='+time_token_latest, '_blank'); 

                    if(apic_platform == "cloud")
                        window.open('/app/cisco-app-ACI/cloud_topology?form.apic_host='+apic_host+'&form.timeToken.earliest='+time_token_earliest+'&form.timeToken.latest='+time_token_latest, '_blank'); 
                });
            });
        });
});