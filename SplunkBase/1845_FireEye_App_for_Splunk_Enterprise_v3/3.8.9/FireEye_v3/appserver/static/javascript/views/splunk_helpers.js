...
// Process Helpers
// ---------------------
async function fetch_conf_data(
  ...
  if (!configuration_file_exist) {
      throw new Error(configuration_file_exist + " does not exist!");
  ...
  var config_stanza_properties = configuration_file_accessor.properties();
  var stanza_name_org = config_stanza_properties['entry'][0]['name'];
  if (stanza_name_org === stanza_name) {
    var stanza_content = config_stanza_properties['entry'][0]['content'];
    return stanza_content
    // throw new Error("Stanza name is :" + JSON.stringify(stanza_content));
  } 
  else
  {
      throw new Error(stanza_name + " does not exist!");
  };
