...
class F5TaskHandler(AdminExternalHandler):
    ...
    def get_input_items(self, session_key, input_value):
        ...
        try:
            ...
            logger.error("Error while fetching details: {}".format(e))
    ...
    def handleRemove(self, confInfo):
        ...
        icontrol_collector = collector.IcontrolCollector(
            session_key, input_item, "disabled"
        )
        try:
        ...
        if not failed_api_call:
            logger.info("Successfully disabled the API calls. Deleting the checkpoint")
            try:
                # Delete checkpoint
                common_utility.delete_checkpoint(session_key, input_name)
                logger.info("Checkpoint deleted for the input: {}".format(input_name))
