...
def get_ssl_value(session_key, stanza_name):
    try:
...
def validate_f5_bigip(session_key, f5_bigip_url, username, password):
    """
    ...
    ssl_value = get_ssl_value(session_key, "ssl_verify")
    try:
        r = requests.post(url=url, data=json.dumps(data), verify=ssl_value, timeout=20)
        return r
    except Exception as e:
        raise Exception(e)
...
def checkpoint_handler(session_key, check_point_key, data):
    """
    Handles checkpoint
    """
...
def check_if_checkpoint_exist(session_key, check_point_key):
    try:
        checkpoint_collection = checkpointer.KVStoreCheckpointer(
            CHECKPOINTER, session_key, ta_name
        )
