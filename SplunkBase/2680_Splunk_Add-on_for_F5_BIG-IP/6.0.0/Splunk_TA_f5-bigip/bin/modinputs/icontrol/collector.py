...
class ServerManager:
    ...
    def get_servers(self, session_key):
        ...
        for server_name in self.server_names:
            try:
                server_obj = Server().server_obj(server_name, session_key)
...
class TemplateManager:
    ...
    def get_templates(self, session_key):
        ...
        for template_name in self.template_names:
            try:
                template_obj = Template().template_obj(template_name, session_key)
            except ValueError as ve:
                logger.error(ve)
                continue
...
class IcontrolCollector:
    ...
    def __init__(self, session_key, input_item, checkpoint_operation):
        self.session_key = session_key
    ...
    def input(self):
        ...
        if self._input is None:
            self._input = Input(self.f5_items).input_item
    ...
    def servers(self):
        ...
        if self._servers is None:
            self._servers = ServerManager(self.input.get("servers"))
    ...
    def hec_info(self):
        ...
        if self._hec_info is None:
            try:
                self._hec_info = HECCollector(self.session_key).get_hec_configuration(
    ...
    def run(self, get_checkpoint=False):
        ...
        checkpoint_object = Checkpointer(self.session_key, {})
        ...
        try:
            ...
            common_utility.checkpoint_handler(
                self.session_key, input_name, str(encoded_value)
            )
        except Exception as e:
            logger.error(
        ...
        if checkpoint_object.checkpoint_value:
            ...
            if (
                ...
                try:
                    ...
                    common_utility.checkpoint_handler(
                        self.session_key, input_name, str(encode_value)
                    )
                except Exception as e:
                    logger.error(
        ...
        for task in tasks:
            task_object = F5BigIPTasks(
                self.session_key, task, self.checkpoint_operation, checkpoint_object
            )
            ...
            logger.info(
                "Data collection completed for template: {} for server {}".format(
        ...
        if checkpoint_object.update_checkpoint:
            ...
            try:
                ...
                logger.error("Error occured while updating checkpoint: {}".format(e))
        else:
            logger.info("No need to update the checkpoint as all the params are same.")
        ...
        if self.checkpoint_operation in ["enabled", "disabled"] and checkpoint_value:
            ...
            try:
                ...
                logger.error("Error in different namespaces: {}".format(e))
            logger.info(
            ...
            if diff_namespaces:
                for namespace in diff_namespaces:
                    ...
                    splunk_host = self._hec_info.get("splunk_host")
                    ...
                    data_collector_object = DataCollector(feeder, "false")
                    response = data_collector_object.make_api_call(logger)
                    if response == 200:
        ...
        logger.info("Data collection completed for input: {}".format(input_name))
...
class F5BigIPTasks:
    ...
    def __init__(self, session_key, task, checkpoint_operation, checkpoint_object):
        self.session_key = session_key
        self.task = task
        self.checkpoint_operation = checkpoint_operation
    ...
    def create_objects(self, failed_api_call):
        for template in self.task.template.keys():
            logger.info("Collecting data for template: {}".format(template))
            for api_info in self.task.template[template].get("api_detail"):
                feeder = Feeder(self.session_key, self.task, api_info)
                ...
                try:
                    if self.checkpoint_operation == "enabled":
                        ...
                        data_collector_object = DataCollector(feeder, "true")
...
class Feeder:
    def __init__(self, session_key, task, api_info):
        ...
        self.ssl_value = common_utility.get_ssl_value(session_key, "ssl_verify")
