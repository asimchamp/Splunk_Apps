...
def getRoutes(url='',username='',password=''):
    ...
    for k ,v in doc['ins_api']['outputs']['output']['body']['TABLE_vrf']['ROW_vrf']['TABLE_addrf']['ROW_addrf']['TABLE_prefix'].items():
            docsub = v
    ...
    for prefix_row in docsub: #executes once for every prefix
        thisPrefix = Prefix()
        for key in list(prefix_row.keys()):  # a simple display of keys and their values
            ...
            if item_type == OrderedDict:
                ...
                for t_key in list(prefix_row[key].keys()):
                    if type(prefix_row[key][t_key]) in [str,unicode]:
                        ...
                        thisNextHop = NextHop()
                        for tr_key in list(prefix_row[key][t_key].keys()):
                            if tr_key == 'ipnexthop':
