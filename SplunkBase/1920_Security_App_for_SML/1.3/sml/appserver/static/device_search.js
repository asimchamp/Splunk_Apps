...
require([
    ...
    function expand_file_field(kwd_search){
        if (!kwd_search){
            return "";
        }
        var f0 = kwd_search.replace(/__FIELD__/g, "file0");
        var f1 = kwd_search.replace(/__FIELD__/g, "file1");
        return "(" + f0 + ") OR (" + f1 + ")";
    }

    function update_input_kwds(val){
        var search = get_search_string(val, '__FIELD__');
        search = expand_file_field(search);
