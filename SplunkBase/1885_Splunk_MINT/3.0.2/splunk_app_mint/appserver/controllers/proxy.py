...
_PROXY_INCLUDE_LIST = [
    {'endpoint': 'data/inputs/mi_cds', 'methods': ['POST', 'GET', 'DELETE']},
    {'endpoint': 'data/inputs/mi_cds/[^/]*', 'methods': ['POST', 'GET', 'DELETE']},
    {'endpoint': 'data/inputs/mi_cds/[^/]*/acl', 'methods': ['POST', 'GET', 'DELETE']},
    {'endpoint': 'data/inputs/mi_cds/[^/]*/enable', 'methods': ['POST']},
    {'endpoint': 'data/inputs/mi_cds/[^/]*/disable', 'methods': ['POST']},

    {'endpoint': 'mint/(?:tokens|projects)', 'methods': ['POST', 'GET', 'DELETE']},
    {'endpoint': 'mint/(?:tokens|projects)/[^/]*', 'methods': ['POST', 'GET', 'DELETE']},
    {'endpoint': 'mint/(?:tokens|projects)/[^/]+/acl', 'methods': ['POST','GET', 'DELETE']}
]

def precompile_includelist():
    for props in _PROXY_INCLUDE_LIST:
        regex_string = '(^|^services/|^servicesNS/[^/]+/[^/]+/)%s$' % props['endpoint']
        regex = re.compile(regex_string)
        props['endpoint'] = regex

precompile_includelist()
...
class ProxyManager(controllers.BaseController):
    ...
    def applyProxy(self, **args):
        ...
        try:
            ...
            uri = urllib.parse.quote(relative_uri)
        logger.debug('[Proxy Traffic] %s request to: %s' % (cherrypy.request.method, uri))
        rawResult = True
        ...
        if cherrypy.request.method in ('POST', 'PUT'):
            ...
            if not content_type or content_type.find('application/x-www-form-urlencoded') > -1:
                ...
                try:
                    ...
                    postargs = cherrypy.request.params
                logger.debug('[Splunkweb Proxy Traffic] request body: %s' % postargs)
            else:
    ...
    def getAllowedEndpointProps(self, uri, method):
        '''verify that that a given uri and associated method is in include list to be proxied to the endpoint.'''
        for props in _PROXY_INCLUDE_LIST:
