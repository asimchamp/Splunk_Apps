...
import logging
logger = logging.getLogger('splunk.module.DonutChart')

import math
import cgi

class DonutChart(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, count=1000, 
        ...
        if not sid:
            raise Exception('DonutChart.generateResults - sid not passed!')

        try:
            job = splunk.search.getJob(sid)
        except splunk.ResourceNotFound, e:
            logger.error('DonutChart could not find the job %s. Exception: %s' % (sid, e))
        ...
        for i, result in enumerate(dataset):
            tdict = {}
            tdict['categories'] = str(result.get('categories', None))
            tdict['data'] = str(result.get('data', None))
