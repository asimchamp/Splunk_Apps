...
require([
    ...
    function setupPopupMenus() {
        var rare10Applications = mvc.Components.getInstance("rare_10_applications");
        var top10Applications = mvc.Components.getInstance("top_10_applications");
        var suspiciousApplications = mvc.Components.getInstance("suspicious_applications");
        var offHoursApplications = mvc.Components.getInstance("off_hours_applications");
        var md5Reuse = mvc.Components.getInstance("md5_reuse");
        ...
        includeExcludeDrilldownMenu.push({
            ...
            search: "index=`index_macro` sourcetype=`process_sourcetype` $criteria$ | table _time Event_ID User_Name, Computer_Name, Application, Operation, Email_Domain, DNS_Hostname, Source_Directory, Source_File_Extension, Destination_Directory, Destination_File_Extension"
        })

        context.init({preventDoubleContext: false});
        context.attachToChart(rare10Applications, includeExcludeMenu);
        context.attachToChart(top10Applications, includeExcludeMenu);
        context.attachToTable(suspiciousApplications, includeExcludeDrilldownMenu);
        context.attachToTable(offHoursApplications, includeExcludeDrilldownMenu);
        context.attachToTable(md5Reuse, includeExcludeDrilldownMenu);
