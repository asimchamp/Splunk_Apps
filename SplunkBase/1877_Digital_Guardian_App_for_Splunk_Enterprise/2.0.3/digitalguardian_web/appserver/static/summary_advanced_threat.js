...
require([
    ...
    function setupPopupMenus() {
        ...
        var bySeverity = mvc.Components.getInstance("by_severity");
        ...
        includeExcludeDrilldownMenu.push({
            ...
            search: "index=`index_macro` sourcetype=`alerts_sourcetype` Rule=ATP* OR Rule=\"[ATP*\" $criteria$ | table _time Event_ID User_Name, Computer_Name, Application, Operation, Email_Domain, DNS_Hostname, Source_Directory, Source_File_Extension, Destination_Directory, Destination_File_Extension"
        })

        context.init({preventDoubleContext: false});
        context.attachToChart(alertsByDay, includeExcludeMenu);
        context.attachToTable(bySeverity, includeExcludeDrilldownMenu);
