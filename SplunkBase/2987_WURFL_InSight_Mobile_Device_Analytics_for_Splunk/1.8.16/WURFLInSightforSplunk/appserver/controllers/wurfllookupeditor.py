...
def setup_logger(level):
    ...
    logger = logging.getLogger('splunk.appserver.WURFLInSightforSplunk.controllers.wurfllookupeditor')
    logger.propagate = False # Prevent the log messages from being duplicated in the python.log file
    logger.setLevel(level)

    file_handler = logging.handlers.RotatingFileHandler(make_splunkhome_path(['var', 'log', 'splunk', 'wurfllookupeditorcontroller.log']), maxBytes=25000000, backupCount=5)
...
class LookupEditor(controllers.BaseController):
    ...
    def save(self, namespace, existingCapabilities, selectedCapabilities):
        ...
        logger.info("selectedCapabilities_string: %s \n type", selectedCapabilities_string,type(selectedCapabilities_string))
        ...
        try:
            ...
            destination_full_path_default = make_splunkhome_path(['etc', 'apps', namespace, 'default', 'transforms.conf'])
            ...
            if os.path.exists(destination_full_path_local) :
                ...
                if config.has_option("wurfl_lookup","fields_list") and config.has_option("wurfl_lookup","external_cmd") :
            ...
            if fieldsFound == False :
                #local/transforms is not found or found but option is not available in the section so accessing from default folder
                # Parsing the transforms configuration file 
                config = ConfigParser.ConfigParser()
            ...
            if fieldsFound == True and not (config is None) and not(destination_full_path is None):
                ...
                config.set("wurfl_lookup", "fields_list",unicode(fields_list))
                # setting value to external_cmd option under wurfl_lookup section	
                config.set("wurfl_lookup", "external_cmd",unicode(externalCommand))
