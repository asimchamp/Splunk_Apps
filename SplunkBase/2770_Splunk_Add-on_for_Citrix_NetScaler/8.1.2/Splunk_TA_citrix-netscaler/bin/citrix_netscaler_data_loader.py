...
class CitrixNetscaler:
    def __init__(self, config):
        ...
        self._http = rest.build_http_connection(config)
    ...
    def _do_request(self):
        ...
        for _ in range(2):
            try:
                resp, content = self._http.request(
                    self._full_url, method="GET", headers=self._headers
                )
            except http.client.ResponseNotReady:
                self._http = rest.build_http_connection(self._config)
        ...
        _LOGGER.debug("end %s", self._full_url)
