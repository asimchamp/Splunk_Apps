...
logger = logging.getLogger("splunk_ta_cisco_ucs_migration")
...
class MigrateExistingPasswords():
  ...
  def update_server_conf(self, session_key, creds_list):
    ...
    try:
      cfm = conf_manager.ConfManager(session_key, constants.TA_NAME, realm="__REST_CREDENTIAL__#{}#configs/conf-{}".format(constants.TA_NAME, constants.SERVERS_CONF))
      servers_conf_exist = utility.file_exist(constants.SERVERS_CONF, constants.TA_NAME)
      ...
      for value in creds_list:
        server_details = {key:val for key, val in value.items() if key != 'name'}
        server_name = value["name"]
        servers_conf_obj.update(server_name, server_details, ['account_password'])

    except Exception:
      logger.error("Exception occured while getting servers_conf object. {} ".format(traceback.format_exc()))
  ...
  def get_credentials(self, session_key):
    ...
    try:
      logger.info("Proceeding to get the server details from all cisco_ucs_servers.conf")
      creds_list = []
      _, response_content = rest.simpleRequest("/servicesNS/nobody/-/configs/conf-cisco_ucs_servers", sessionKey=session_key, getargs={"output_mode": "json"})
      ...
      for each_server in json_obj:
        ...
        if each_server.get("content"):
          ...
          if content.get('disable_ssl_verification'):
            creds_dict['disable_ssl_verification'] = content['disable_ssl_verification']
          if content.get("account_name") == "******" and content.get("account_password") == "******":
            search_string = "_" + content["eai:appName"] + "_account_#" + content["eai:appName"] + "#" + server_name
            ...
            for stanza, value in entities.items():
              if "clear_password" in value:
                ...
                creds_dict['account_password'] = clear_password_splitted[1]
          else:
            logger.debug('credentials were not encrypted for the server "{}" in the app {}'.format(server_name, content["eai:appName"]))
      ...
      logger.error("error while retrieveing server details from the cisco_ucs_servers.conf. {} ".format(traceback.format_exc()))
  ...
  def migrate_existing_passwords(self):
    ...
    try:
      ...
      setting_conf_exist = utility.file_exist(constants.SETTINGS_CONF_FILE, constants.TA_NAME)
      if not setting_conf_exist:
        utility.create_splunk_ta_cisco_ucs_settings_conf_file(cfm, constants.PASSWORDS_MIGRATION_STANZA, session_key)
      else:
        has_migrated = utility.check_has_migrated_value(cfm, constants.PASSWORDS_MIGRATION_STANZA)

      if has_migrated == "0":
        logger.debug("Passwords have not been migrated yet. Proceeding to migrate passwords.")
        creds = self.get_credentials(session_key)
        self.update_server_conf(session_key, creds)
        utility.update_settings_conf(session_key, constants.PASSWORDS_MIGRATION_STANZA)
        logger.info("Passwords migration has been completed. Servers are now stored in local/{}.conf".format(constants.SERVERS_CONF))
    except Exception:
      logger.error("Error while migrating passwords. {} ".format(traceback.format_exc()))
