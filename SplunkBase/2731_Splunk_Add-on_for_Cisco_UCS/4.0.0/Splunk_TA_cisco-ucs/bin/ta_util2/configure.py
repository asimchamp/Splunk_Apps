...
def _parse_modinput_configs(root, outer_block, inner_block):
    ...
    if not confs:
        ...
        raise Exception("Invalid config, missing {} section".format(
            outer_block
        ))
...
def get_modinput_configs(modinput, modinput_stanza=None):
    ...
    out, err = subprocess.Popen(cli, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate() # nosemgrep false-positive: The vulnerable method get_modinput_configs in the ta_util2 framework file is not getting used anywhere in the add-on v3.1.0. 
    if err:
