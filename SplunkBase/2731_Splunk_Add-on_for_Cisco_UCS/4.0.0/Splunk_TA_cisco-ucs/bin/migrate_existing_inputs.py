...
class MigrateExistingInputs():
  ...
  def get_tasks(self, session_key):
    ...
    """
    try:
      response_code, repsonse_content = rest.simpleRequest("/servicesNS/nobody/-/configs/conf-cisco_ucs_tasks", sessionKey=session_key, getargs={"output_mode": "json", "count": "0"})
      json_content = json.loads(repsonse_content)
      tasks_obj = json_content.get("entry")
      return tasks_obj
    except Exception:
      logger.error("Error while getting tasks from cisco_ucs_tasks.conf file. {}".format(traceback.format_exc()))
    ...
    try:
      ...
      for task in tasks_list:
        ...
        if task_content.get('templates'):
          new_dict['templates'] = self.form_new_value(task_content['templates'])
  ...
  def transfer_existing_inputs(self):
    ...
    try:
      ...
      if not settings_file_exists:
        ...
        utility.create_splunk_ta_cisco_ucs_settings_conf_file(cfm, constants.INPUTS_MIGRATION_STANZA, session_key)
      else:
        has_migrated = utility.check_has_migrated_value(cfm, constants.INPUTS_MIGRATION_STANZA)
      if has_migrated == "0":
        tasks_list = self.get_tasks(session_key)
        self.transfer_data_to_inputs(session_key, tasks_list)
        utility.update_settings_conf(session_key, constants.INPUTS_MIGRATION_STANZA)
        logger.info("Tasks migration has been completed.")
    except Exception:
      logger.error("Error while migrating tasks. {} ".format(traceback.format_exc()))
