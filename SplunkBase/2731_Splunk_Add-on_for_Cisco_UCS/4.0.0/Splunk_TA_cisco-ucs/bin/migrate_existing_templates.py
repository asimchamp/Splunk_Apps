...
class MigrateExistingTemplates():
  ...
  def get_templates(self, session_key):
    ...
    """
    try:
      response_code, repsonse_content = rest.simpleRequest("/servicesNS/nobody/-/configs/conf-cisco_ucs_templates", sessionKey=session_key, getargs={"output_mode": "json", "count": "0"})
      json_content = json.loads(repsonse_content)
      templates_obj = json_content.get("entry")
      return templates_obj
    except Exception:
      logger.error("Error while getting templates from cisco_ucs_templates.conf file. {}".format(traceback.format_exc()))
  ...
  def transfer_data_to_new_templates_conf(self, session_key, templates_list):
    ...
    try:
      ...
      new_templates_file_exists = utility.file_exist(constants.TEMPLATES_CONF, constants.TA_NAME)
      ...
      logger.error("HTTPError: {}".format(traceback.format_exc()))
    except KeyError:
      logger.error("Keyerror exception: {}".format(traceback.format_exc()))
    except Exception:
  ...
  def transfer_existing_templates(self):
    ...
    """
    try:
      logger.debug("Proceeding to migrate existing templates into {}.conf".format(constants.TEMPLATES_CONF))
      dict_templates_transfer = {}
      has_migrated = "0"
      session_key = utility.get_session_key()
      cfm = conf_manager.ConfManager(session_key, constants.TA_NAME)
      settings_file_exists = utility.file_exist(constants.SETTINGS_CONF_FILE, constants.TA_NAME)
      if not settings_file_exists:
        logger.info("settings conf does not exist. Proceeding to create settings conf.")
        utility.create_splunk_ta_cisco_ucs_settings_conf_file(cfm, constants.TEMPLATES_MIGRATION_STANZA, session_key)
      else:
        has_migrated = utility.check_has_migrated_value(cfm, constants.TEMPLATES_MIGRATION_STANZA)
      if has_migrated == "0":
        templates_list = self.get_templates(session_key)
        self.transfer_data_to_new_templates_conf(session_key, templates_list)
        utility.update_settings_conf(session_key, constants.TEMPLATES_MIGRATION_STANZA)
        logger.info("Templates migration has been completed. All templates are now stored in local/{}.conf".format(constants.TEMPLATES_CONF))
    except Exception:
      logger.error("Error while migrating templates. {} ".format(traceback.format_exc()))
