...
logger = logging.getLogger(log_files.splunk_ta_cisco_ucs_migration)
...
def create_splunk_ta_cisco_ucs_settings_conf_file(cfm, stanza, session_key=None):
    ...
    try:
      ...
      cfm_transfer_conf.update("logging", {
        "loglevel" : log_level
      })
      dict_settings_transfer = {
        "has_migrated" : 1
      }
      cfm_transfer_conf.update(constants.SETTINGS_MIGRATION_STANZA, dict_settings_transfer)
      logger.debug("The logging settings have been migrated to the splunk_ta_cisco_ucs_settings.conf")
      
      dict_inputs_transfer['has_migrated'] = 0
      cfm_transfer_conf.update(stanza, dict_inputs_transfer)
    
    except Exception:
      logger.error("Error while creating the transfer_settings.conf file. {}".format(traceback.format_exc()))
...
def get_logging_settings_from_cisco_ucs_conf(session_key):
    ...
    """
    try:
      _, response_content = rest.simpleRequest("/servicesNS/nobody/Splunk_TA_cisco-ucs/configs/conf-cisco_ucs", sessionKey=session_key, getargs={"output_mode": "json"})
      ...
      logger.error("Error while getting logging settings from cisco_ucs.conf file. {}".format(traceback.format_exc()))
...
def check_has_migrated_value(cfm, stanza):
    ...
    try:
      ...
      logger.error("Error inside transfer_settings.conf read: {}".format(traceback.format_exc()))
      return has_migrated
...
def get_session_key():
    ...
    try:
      ...
      logger.error("inside session key exception: {}".format(traceback.format_exc()))
...
def update_settings_conf(session_key, stanza_name):
  ...
  try:
    ...
    logger.error("Exception occured while updating splunk_ta_cisco_ucs_settings.conf stanza {}".format(traceback.format_exc()))
