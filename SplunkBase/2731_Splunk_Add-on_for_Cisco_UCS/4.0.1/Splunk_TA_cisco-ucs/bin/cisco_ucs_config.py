...
class CiscoUcsConfMonitor:
    def __init__(self, callback):
        ...
        conf_files = {
            ...
            op.join(
                app_dir, "default", "{}.conf".format(constants.TEMPLATES_CONF)
            ): None,
            op.join(
                app_dir, "local", "{}.conf".format(constants.SETTINGS_CONF_FILE)
            ): None,
        ...
        for k in conf_files:
            try:
                ...
                _LOGGER.error(
                    "Failure occurred while reading modified time of the conf file '{0}'. The reason for "
