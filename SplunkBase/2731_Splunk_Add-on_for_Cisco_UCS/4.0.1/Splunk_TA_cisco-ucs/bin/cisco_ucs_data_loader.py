...
class CiscoUcs:
    ...
    def collect_data(self):
        if self._lock.locked():
            _LOGGER.info(
                "Last request for %s has not been done yet", self._config["class_ids"]
            )
            return None
    ...
    def _to_xml(err_content):
        if not err_content[0] and err_content[1]:
            ...
            if xml is None:
                _LOGGER.error("Failed to parse XML response %s", err_content[1])
                return None
            return xml
        else:
            return None
        ...
        for retry_count in range(2):
            try:
                resp, content = self._http.request(
                    self._full_url, method=method, headers=headers, body=payload
                )
                if content:
