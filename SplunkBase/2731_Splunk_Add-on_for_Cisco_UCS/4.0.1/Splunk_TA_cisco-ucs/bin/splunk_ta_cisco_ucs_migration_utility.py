...
def create_splunk_ta_cisco_ucs_settings_conf_file(cfm, stanza, session_key=None):
    ...
    try:
        ...
        logger.error(
            "Error while creating the transfer_settings.conf file. {}".format(
...
def get_logging_settings_from_cisco_ucs_conf(session_key):
    ...
    try:
        ...
        logger.error(
            "Error while getting logging settings from cisco_ucs.conf file. {}".format(
                traceback.format_exc()
            )
        )
...
def check_has_migrated_value(cfm, stanza):
    ...
    try:
        ...
        logger.error(
            "Error inside transfer_settings.conf read: {}".format(
                traceback.format_exc()
            )
        )
        return has_migrated
...
def update_settings_conf(session_key, stanza_name):
    ...
    try:
        ...
        logger.error(
            "Exception occured while updating splunk_ta_cisco_ucs_settings.conf stanza {}".format(
                traceback.format_exc()
            )
        )
