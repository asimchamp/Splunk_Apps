...
def _check_duration(duration, input_name):
    ...
    try:
        if 0 < int(duration) < 31536001:
            return True
        else:
            _LOGGER.error(
                "Got unexpected value {} of 'duration' field for input '{}'. "
                "Duration should be in between 1 and 31536000 seconds. "
                "You can either change it in inputs.conf file or edit "
                "'Collection interval' on Inputs page.".format(duration, input_name)
            )
            return False
    except ValueError:
        _LOGGER.error(
            "Got unexpected value {} of 'duration' field for input '{}'. Duration should be an integer. "
...
def proxy_port_value_validation(value):
    ...
    try:
        ...
        _LOGGER.error(
            "Invalid Proxy Port value in inputs.conf file. To start data collection please provide valid proxy port value."  # noqa: E501
        )
        return False
    except TypeError:
        _LOGGER.error(
            "No Proxy Port value in inputs.conf file. To start data collection please provide valid proxy port value."
        )
        return False
    return True
...
def _setup_signal_handler(data_loader):
    ...
    solnlib_utils.handle_teardown_signals(_handle_exit)
...
class SNOW(smi.Script):
    ...
    def _encrypt_credentials(
        self, account_conf_manager, settings_conf_manager, session_key, server_uri
    ):
        """
        ...
        self.encryption_manager = conf.ConfManager(server_uri, session_key)
        ...
        try:
            ...
            _LOGGER.error(
                "Failure occurred in accessing the splunk_ta_snow_account.conf file. Error: {}".format(
                    str(e)
                )
            )
            return
        ...
        try:
            ...
            _LOGGER.error(
                "Failed to encrypt credentials for account '{}'. Exception occurred: {}".format(
                    account, str(e)
                )
            )
            _LOGGER.error(traceback.format_exc())
        ...
        try:
            ...
            _LOGGER.error(
                "Failed to encrypt credentials for proxy configuration. Exception occurred: {}".format(
                    str(e)
                )
            )
            _LOGGER.error(traceback.format_exc())
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
                ...
                try:
                    ...
                    if input_item.get("include"):
                        if not special_character_validation(input_item.get("include")):
                            _LOGGER.error(
                                "In include properties, {}. To resume data collection, "
                    ...
                    if input_item.get("exclude"):
                        if not special_character_validation(input_item.get("exclude")):
                            _LOGGER.error(
                                "In exclude properties, {}. To resume data collection, "
                                "please provide the valid values.".format(
                                    FIELD_VALIDATION_MESSAGE.format(
                                        input_item["exclude"]
                                    )
                    ...
                    if input_item.get("timefield"):
                        if not special_character_validation(
                            input_item.get("timefield")
                        ):
                            _LOGGER.error(
                                "In timefield field, {}. To resume data collection, please provide the valid values."
                                "".format(
                                    FIELD_VALIDATION_MESSAGE.format(
                    ...
                    if input_item.get("id_field"):
                        if not special_character_validation(input_item.get("id_field")):
                            _LOGGER.error(
                                "In id_field field, {}. To resume data collection, please provide the valid values."
                                "".format(
                                    FIELD_VALIDATION_MESSAGE.format(
                                        input_item["id_field"]
                                    )
                    ...
                    if not input_item.get("duration"):
                        ...
                        _LOGGER.warn(
                            "No duration specified for input '{}'."
                            "Using default duration=60 for now.".format(
                                input_item["name"]
                            )
                        )
                    ...
                    if not res:
                        _LOGGER.error(
                            ...
                            "edit it from the Inputs page of the add-on."
                        )
                        continue
                    ...
                    if "record_count" in list(account_details.keys()):
                        ...
                        if account_record_count > 10000 or account_record_count < 1000:
                            _LOGGER.error(
                                "Specified account {} linked to input '{}' has a record count not in range 1000 "
                    ...
                    if account_auth_type == "basic":
                        ...
                        account_parameter = [
                            f
                            for f in snow_oauth_account_fields
                            if f not in account_details
                        ]
                    else:
                    ...
                    input_item["auth_type"] = input_item.get(
            ...
            # semgrep ignore reason: doesn't cause segmentation fault as thread spawning is limited
            data_loader = dl.GlobalDataLoader.get_data_loader(  # nosemgrep: contrib.dlint.dlint-equivalent.insecure-dl-use  # noqa: E501
                meta_configs, stanza_configs, jf.JobFactory()
            )
