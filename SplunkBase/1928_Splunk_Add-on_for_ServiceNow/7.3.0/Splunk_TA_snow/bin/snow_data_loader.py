...
class _ObjectWrapper(object):
    ...
    def to_string(self, idx, host, excludes=()):
        evt_fmt = (
            '<event stanza="snow://{0}"><time>{1}</time><source>{2}</source>'
        ...
        try:
            timestamp = datetime.strptime(self.obj[self.timefield], "%Y-%m-%d %H:%M:%S")
        except ValueError:
            timestamp = datetime.utcnow()
        return evt_fmt.format(
            self.input_name,
            utils.datetime_to_seconds(timestamp),
            self.endpoint,
            "snow:" + self.table,
            host,
            idx,
            self.endpoint,
            utils.escape_cdata(tag_vals),
        )
...
class Snow(object):
        ...
        while True:
            ...
            with self._lock:
                (
                    last_timestamp,
                    next_offset,
                ) = self._read_last_collection_time_and_offset(input_name, timefield)
                ...
                try:
                    ...
                    _LOGGER.error(
                        "Failure occurred while getting records for the table: {1} from {0}. "
                ...
                if jobjs.get("error"):
                    _LOGGER.error(
                        ...
                        "for further information.".format(
                            self.host, table, str(jobjs["error"])
                        )
                    )
                    return
                ...
                self._write_events_to_event_queue(
                    jobjs, table, timefield, input_name, host, excludes
                )
                self.context[checkpoint_file_name]["next_offset"] = next_offset + len(
                    jobjs
                )

                if (
                    self.context[checkpoint_file_name].get("last_time_records")
                    and jobjs
                ):
                    # setting offset using older version of checkpoint if any exist.
                    # this is used to minimize data duplication during upgrade.
                    (
                        self.context[checkpoint_file_name]["last_time_records"],
                        jobjs,
    ...
    def _write_events_to_event_queue(
        self, jobjs, table, timefield, input_name, host, excludes
    ):
        """
        ...
        objs.extend(
            (
                _ObjectWrapper(obj, table, self.host, timefield, input_name)
                for obj in jobjs
            )
        )
        results = {table: objs}
            ...
            if data_objs:
                events = "".join(
                    (
                        "<stream>%s</stream>"
                        % obj.to_string(
                            self.config.get("index", "default"), host, excludes
                        )
    ...
    def _read_last_collection_time_and_offset(self, input_name, timefield):
        ...
        if not self.context.get(checkpoint_file_name, None):
            _LOGGER.debug(
                "Checkpoint file {} not in cache, reloading from checkpoint file".format(
        ...
        return self.context[checkpoint_file_name]["last_collection_time"].replace(
            " ", "+"
        ), self.context[checkpoint_file_name].get("next_offset", 0)

    def _read_last_checkpoint(self, input_name):
        fname = op.join(self.checkpoint_dir, input_name)
        if not op.isfile(fname):
            _LOGGER.debug(
                "Checkpoint file not found for the input. Will use configured start date"
            )
            return None
        try:
            ...
            _LOGGER.error(
                "Error while reading last checkpoint. {}".format(traceback.format_exc())
            )
            return None
