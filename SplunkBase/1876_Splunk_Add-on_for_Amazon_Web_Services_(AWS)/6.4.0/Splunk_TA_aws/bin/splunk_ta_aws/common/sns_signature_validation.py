...
def get_certificate(url):
    ...
    if url not in cache_certificates:
        ...
        with urllib.request.urlopen(url) as file:
            set_certificate_cache(url, file.read())  # pylint: disable=R1732
...
def sqs_validate(message):
    ...
    if validate_aws_sns_message_validate:
        ...
        validate_aws_sns_message_validate.validate(
            message, get_certificate=get_certificate
        )
    else:
        logger.warning(
            f"crypto is not loading ignoring SNS validation: {message.get('message_id')}"
        )
