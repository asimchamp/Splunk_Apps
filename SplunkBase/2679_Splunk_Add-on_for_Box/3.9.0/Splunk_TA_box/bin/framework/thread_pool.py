...
class ThreadPool:
    ...
    def __init__(self, min_size=1, max_size=128, task_queue_size=256, daemon=True):
        assert task_queue_size  # nosemgrep: gitlab.bandit.B101
    ...
    def tear_down(self):
        ...
        with self._lock:
            ...
            for thr in self.thrs:
                self.work_queue.put(False, block=False)
    ...
    def enqueue_jobs(self, jobs, block=True):
        ...
        for job in jobs:
            if job is None:
                self.work_queue.put(False, block)
            else:
    ...
    def resize(self, new_size):
        ...
        with self._lock:
            ...
            if new_size > size:
                ...
                for _ in range(int(size) - int(new_size)):
                    self.work_queue.put(False, block=False)
