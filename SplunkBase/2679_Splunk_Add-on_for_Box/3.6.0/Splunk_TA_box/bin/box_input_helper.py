...
def collect_endpoint_data(
    ...
    params = {}
    checkpoint = ckp_manager.get(input_name)
    ...
    while not stop_flag:
        try:
            raw_data = box_helper.fetch_data(
                client, box_helper.fetch_stream_event_uri(params)
            )
        except Exception as err:
            raise err
        ...
        checkpoint = ckp_manager.update(input_name, new_checkpoint)
...
def stream_events(helper, inputs, ew):
    ...
    try:
        ...
        ckp_manager = FileCheckpointer(inputs.metadata["checkpoint_dir"])
        ...
        utils.setup_logging("ta_box_live_monitor", loglevel, refresh=False)
