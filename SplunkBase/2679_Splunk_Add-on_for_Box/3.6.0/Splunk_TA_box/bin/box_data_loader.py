...
class BoxBase:
    ...
    def _check_if_job_is_due(self):
        ...
        if ckpt is not None and ckpt["ckpts"] is None:
            ...
            if int(delta) > 0:
                _LOGGER.info(
                    "There are %f seconds for the job=%s to be due",
                    delta,
                    self.config["rest_endpoint"],
                )
                return False
    ...
    def _save_state(self, ckpts, start_time=None):
        old_ckpt = self.config["state_store"].get_state(self.config["input_name"])
        ...
        self.config["state_store"].update_state(self.config["input_name"], ckpt)

    def _remove_state(self):
        self.config["state_store"].delete_state(self.config["input_name"])

    def _get_state(self):
        ckpt = self.config["state_store"].get_state(self.config["input_name"])
...
class BoxUserGroupBase(BoxBase):
    ...
    def _save_ckpts(self, result):
        if not result or not result["entries"]:
            _LOGGER.info(
                "All %s records have been collected", self.config["rest_endpoint"]
            )
