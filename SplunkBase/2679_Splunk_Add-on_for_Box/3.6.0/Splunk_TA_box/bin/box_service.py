...
def _get_account_id(
    proxy_configuration, access_token, disable_ssl_validation, session_key
):
    try:
        ...
        _LOGGER.error("Failed to get account id. Reason=%s", traceback.format_exc())
        return None
        ...
        try:
            ...
            for input_name, input_item in inputs.inputs.items():  # py2/3
                ...
                account_id = _get_account_id(
                    splunk_ta_box_settings_conf.get("proxy"),
                    splunk_ta_box_account_conf[input_item["account"]]["access_token"],
                    input_item["disable_ssl_certificate_validation"],
                    meta_configs[SESSION_KEY],
                )
