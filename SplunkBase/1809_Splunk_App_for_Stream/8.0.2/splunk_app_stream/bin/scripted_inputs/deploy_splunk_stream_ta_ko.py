...
def main():
    ...
    if isStreamTABinaryAvailable():
        ...
        if isStreamTADisabled(sessionToken):
            reload = disablePCAPUpload(sessionToken)
        else:
            logger.info("Splunk App for Stream Dependency Manager: Enabling PCAP Upload modinput since Stream TA binaries are available...")
            reload = enablePCAPUpload(sessionToken)
    else:
