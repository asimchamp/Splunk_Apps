				...
				var metaTags = gistFile.find("div.gist-meta a").filter(
					function() {
						...
						return($(this).text().search(
							new RegExp("^\\s*(view raw|this gist|github)", "i")) === -1);
					});
...
$(document).ready(function() {
	var msmqint = $("#msmq-handler-interface");
	var msmqintResult = $.getGist("6128867");
	msmqintResult.done(function(gist) {
		console.log("GIST Received = %o", gist);
		msmqint.empty();
		var ordered = gist.ordered;
		for (var i = 0 ; i < ordered.length ; i++) {
			msmqint.append("<h3>" + ordered[i].fileName + "</h3>");
			msmqint.append(ordered[i].content);
		}
	});
});