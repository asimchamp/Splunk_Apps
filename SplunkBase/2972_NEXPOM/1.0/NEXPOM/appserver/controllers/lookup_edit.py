...
def setup_logger(level):
    ...
    logger = logging.getLogger('splunk.appserver.NEXPOM.controllers.LookupEditor')
    logger.propagate = False # Prevent the log messages from being duplicated in the python.log file
    logger.setLevel(level)

    file_handler = logging.handlers.RotatingFileHandler(make_splunkhome_path(['var', 'log', 'splunk', 'NEXPOM_controller.log']), maxBytes=25000000, backupCount=5)
...
class LookupEditor(controllers.BaseController):
    ...
    def get_lookup_info(self, lookup_file, namespace="NEXPOM", **kwargs):
        """
    ...
    def make_conf(self, lookup_file, namespace, **kwargs):
        logger.info("make_inputs_conf start")
        logger.info(lookup_file)
        logger.info(namespace)
        
        # Note that this path will only be used if the lookup file doesn't already exist
        destination_full_path = make_splunkhome_path(['etc', 'apps', namespace, 'data_file', lookup_file])
        logger.info(destination_full_path)
        # Make the lookups directory if it does not exist
        destination_lookups_path = make_splunkhome_path(['etc', 'apps', namespace, 'data_file'])
        input_conf_path = make_splunkhome_path(['etc','apps', namespace, 'local', 'inputs.conf'])
        props_conf_path = make_splunkhome_path(['etc','apps', namespace, 'local', 'props.conf'])

        # Inputs conf Make
        f = open(input_conf_path, 'a')
        s = os.path.splitext(lookup_file)
        sourcetype=s[0].replace(' ', '_')

        f.write("\n\n[monitor://"+destination_full_path+"]")
        f.write("\ndisabled = false")
        f.write("\nindex="+namespace)
    ...
    @expose_page(must_login=True, methods=['GET']) 
    def get_original_lookup_file(self, lookup_file, namespace="NEXPOM", **kwargs):
        """
    ...
    def resolve_lookup_filename(self, lookup_file, namespace="NEXPOM", owner=None, get_default_csv=True, version=None, throw_not_found=True):
        """
        ...
        if version is not None and owner is not None:
            ...
            lookup_path_default = make_splunkhome_path(["etc", "users", owner, namespace, "data_file", lookup_file + ".default"])
        elif version is not None:
            lookup_path = make_splunkhome_path([self.getBackupDirectory(lookup_file, namespace, owner, resolved_lookup_path=resolved_lookup_path), version])
            lookup_path_default = make_splunkhome_path(["etc", "apps", namespace, "data_file", lookup_file + ".default"])
        elif owner is not None:
            # e.g. $SPLUNK_HOME/etc/users/luke/SA-NetworkProtection/lookups/test.csv
            lookup_path = resolved_lookup_path
            #lookup_path = make_splunkhome_path(["etc", "users", owner, namespace, "data_file", lookup_file])
            lookup_path_default = make_splunkhome_path(["etc", "users", owner, namespace, "data_file", lookup_file + ".default"])
        else:
            lookup_path = resolved_lookup_path
            #lookup_path = make_splunkhome_path(["etc", "apps", namespace, "data_file", lookup_file])
            lookup_path_default = make_splunkhome_path(["etc", "apps", namespace, "data_file", lookup_file + ".default"])
    ...
    def get_lookup(self, lookup_file, namespace="NEXPOM", owner=None, get_default_csv=True, version=None):
        """
    ...
    @expose_page(must_login=True, methods=['GET']) 
    def get_lookup_contents(self, lookup_file, namespace="NEXPOM", owner=None, header_only=False, version=None, **kwargs):
        """
    ...
    @expose_page(must_login=True, methods=['GET']) 
    def get_lookup_header(self, lookup_file, namespace="NEXPOM", owner=None, **kwargs):
        pass
