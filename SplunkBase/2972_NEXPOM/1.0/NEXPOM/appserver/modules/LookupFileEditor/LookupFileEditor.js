...
function doSaveLookup(){
	...
	$.ajax( Splunk.util.make_url('/custom/NEXPOM/lookup_edit/save'),
			{
				uri:  Splunk.util.make_url('/custom/NEXPOM/lookup_edit/save'),
				type: 'POST',
				data: data,
				...
				complete: function(jqXHR, textStatus){
					...
					console.info("  v completed in " + elapsed + "ms");
					var success = true;
					...
					if(success){
						makeInputsConf(data.lookup_file, data.namespace);
						console.log("test");
						showDefaultContent();
						$("#lookup_file_input").val("");
						$("#importLookup").val("");
...
function getLookupFileInfo(lookup_file, namespace, user){
	...
	// Make the URL
	url = Splunk.util.make_full_url("/custom/NEXPOM/lookup_edit/get_lookup_info", data);
...
$(document).ready(
		function(){
			addStylesheet('/static/app/NEXPOM/css/lib/jquery.handsontable.full.css'); //This is necessary for Splunk 5.0.x support
			setupHandlers();
		}
);
...
function loadLookupBackupsList(lookup_file, namespace, user){
	...
	// Make the URL
    url = Splunk.util.make_full_url("/custom/NEXPOM/lookup_edit/get_lookup_backups_list", data);
...
function loadLookupContents(lookup_file, namespace, user, header_only, version){
	...
	// Make the URL
    url = Splunk.util.make_full_url("/custom/NEXPOM/lookup_edit/get_lookup_contents", data);
...
function makeInputsConf(lookup_file, namespace){
	...
	url = Splunk.util.make_full_url("/custom/NEXPOM/lookup_edit/make_conf", data);
	// Perform the call
	$.ajax({
		  url: url,
		  type: "POST",
		  ...
		  // On success, populate the table
		  success: function(data) { 
			  console.info('Conffile has been created correctly');
		  },
...
function refresh(){
	...
	url = Splunk.util.make_full_url("/debug/refresh");
    console.log(url);
	// Perform the call
	$.ajax({
		  url: url,
...
Splunk.Module.LookupFileEditor = $.klass(Splunk.Module, {
                ...
                $(this).ajaxSubmit({
                    ...
                    'success': function(json) {
                        ...
                        if (json["success"]) {
                        	...
                        	messenger.send('info', "splunk.NEXPOM", json["message"]);
                        	
                        // If it was unsuccessful, then print an error message accordingly
                        } else {
                            messenger = Splunk.Messenger.System.getInstance();
                            messenger.send('error', "splunk.NEXPOM", _('ERROR - ') + json["message"] || json);
                        }
                    },
                    'dataType': 'json'
                });
    ...
    handleSubmitCallback: function() {
    	var messenger = Splunk.Messenger.System.getInstance();
    	messenger.send('info', "splunk.NEXPOM", "Action succeeded");
