...
class JMXOpInvoker:
    ...
    def start(self):
        ...
        for program_input in program_inputs:
            ...
            log4j2_xml_path = os.path.sep.join([self._OP_INVOKE_HOME, "config", "log4j2.xml"])

            vm_arguments = {c.LOG_PATH_PARAMS: c.LOG_PATH, c.LOG_LEVEL_PARAMS: c.LOG_LEVEL_VALUE, c.LOG4J_2_PROP_FILE : log4j2_xml_path}
    ...
    def _get_valid_stanzas(cls, stanzas):
        ...
        for stanza in stanzas:
            if not cls._validate_stanza(stanza):
                cls._LOGGER.error('Stanza ' + stanza.get(c.NAME, 'None') + ' is invalid')
                continue
    ...
    def _validate_stanza(cls, stanza):
        if any((not stanza[k] for k in (c.DURATION, c.NAME, c.JMX_URL, c.OBJECT_NAME,
                                        c.OPERATION_NAME))):
            return False
        return True
    ...
    def _create_state_store(self):
        state_store = get_state_store(meta_configs=self._meta, appname=c.APP_NAME, use_kv_store=False)
        return state_store
    ...
    def _start_scheduler(self):
        ...
        for stanza in self._stanzas:
            ...
            self._timer_queue.add_timer(callback=write_file_task, when=time.time(), interval=int(duration))
        ...
        while True:
            ...
            if output:
                try:
                    ...
                    break
        rc = p.poll()
        self._LOGGER.info('The jmx-op-invoke is finished, the return code is ' + str(rc))
