#
# $Id: Intersplunk.pm,v c0f82dbf461e 2019/03/19 11:23:56 gomor $
#
...
my $output_debug;
if ($Debug) {
   eval("use Data::Dumper;");  # Avoid all-time dep.
   open($input_debug, '>>', '/tmp/input-intersplunk-debug.txt');
   open($output_debug, '>>', '/tmp/output-intersplunk-debug.txt');
}
...
sub readResults {
   ...
   # Activate only to dump the comple input results.
   #open(my $input, '>', '/tmp/input-intersplunk.txt');

   if ($has_header) {
      if ($Debug) {
         while (my $line = <$stdin>) {
            # Activate only to dump the complete input results.
            #print $input $line;
   ...
   while (my $line = $csvr->getline($stdin)) {
      ...
      if ($first) {
         ...
         # Check which fields are multivalued (for a field
         # 'foo', '__mv_foo' also exists)
      ...
      for my $key (@mv_fields) {
         ...
         &&  exists($result->[$lookup->{$mv_key}])) {
            # Expand the value of __mv_[key] to a list, store it in key,
            # and delete __mv_[key]
            my $vals = [];
...
sub isGetInfo {
   ...
   #print $output_debug Data::Dumper::Dumper($args)."\n";
...
sub outputResults {
   ...
   #print $output_debug Data::Dumper::Dumper($messages)."\n";
   # Example:
   # $messages = {
   #    streaming_preop' => '0',
   #    streaming' => '0',
   ...
   #    clear_required_fields' => '0',
   # };
   #print $output_debug Data::Dumper::Dumper($results)."\n";
   #print $output_debug Data::Dumper::Dumper($header)."\n";
   # Example:
   # $VAR1 = [
   #       'new',
   #       '_time',
   ...
   #       '_raw'
   #     ];
   #print $output_debug Data::Dumper::Dumper($lookup)."\n";
   # Example:
   # $VAR1 = {
   #       'test' => 3,
   #       'tes2' => 2,
   ...
   if (defined($messages)) {
      # message header is everything before the first empty line,
      # similar to the input header format. also key = value, with
