...
require(['jquery', 'underscore', 'splunkjs/mvc', 'splunkjs/mvc/tableview', 'splunkjs/mvc/simplexml/ready!'], function($, _, mvc, TableView)
{
   var TimebarCellRenderer = TableView.BaseCellRenderer.extend(
   {
      ...
      render: function($td, cell)
      {
         ...
         if (cell.field === 'Lifetime')
         {
            ...
            else if (processType === "Group policy")
            {
               color = "#99712b";
            }
            else if (processType === "Session setup")
            {
               color = "#d6563c";
            }
            else if (processType === "AD logon script")
            {
               color = "#ed8440";
            }
            else if (processType === "GP logon script")
            {
               color = "#6a5c9e";
            }
            else if (processType === "Userinit")
            {
               color = "#31a35f";
            }
            else if (processType === "Shell")
            {
               color = "#31a35f";
            }
            else if (processType === "AppSetup")
            {
               color = "#a2cc3e";
            }
            else if (processType === "Active Setup")
            {
               color = "#cc5068";
            }
            else if (processType === "Run once")
            {
               color = "#f2b827";
            }
   ...
   mvc.Components.get('Table_Panel41').getVisualization(function(tableView)
   {
