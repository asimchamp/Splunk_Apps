         ...
         "splunkjs/mvc/tokenforwarder"], function (mvc)
{
   ...
   if (mode == "app")
   {
      ...
      document.title = document.title.replace (regex, "Application GPU ");
      if (easySplunkVersion >= 70300)
      {
         $('.dashboard-header > h1').text ("Application GPU");
      }
      else
      {
         $('.dashboard-header > h2').text ("Application GPU");
      }
      $('.dashboard-header > p.description').text ("This dashboard displays detailed information about GPU usage per application.");
   ...
   else if (mode == "proc")
   {
      ...
      // Page title and description
      document.title = document.title.replace (regex, "Process GPU ");
      if (easySplunkVersion >= 70300)
      {
         $('.dashboard-header > h1').text ("Process GPU");
      }
      else
      {
         $('.dashboard-header > h2').text ("Process GPU");
      }
      $('.dashboard-header > p.description').text ("This dashboard displays detailed information about GPU usage per process.");
