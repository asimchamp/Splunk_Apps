         ...
         "splunkjs/mvc/tokenforwarder"], function (mvc)
{
   ...
   if (mode == "app")
   {
      ...
      SetToken (mvc, "ModeGroupByAs", gettext("App name(s)"));
      SetToken (mvc, "ModeGroupByPluralLower", gettext("applications"));
      SetToken (mvc, "RadioButtonLabel", gettext("Only show new applications?"));

      // Page title and description
      document.title = document.title.replace (regex, gettext("Application Startup"));
      if (easySplunkVersion >= 70300)
      {
         $('.dashboard-header > h1').text (gettext("Application Startup"));
      }
      else
      {
         $('.dashboard-header > h2').text (gettext("Application Startup"));
      }
      $('.dashboard-header > p.description').text (gettext("This dashboard displays detailed information about application startup performance."));
   }
   else
   {
      //
      ...
      SetToken (mvc, "ModeGroupByAs", gettext("Process name(s)"));
      SetToken (mvc, "ModeGroupByPluralLower", gettext("processes"));
      SetToken (mvc, "RadioButtonLabel", gettext("Only show new processes?"));

      // Page title and description
      document.title = document.title.replace (regex, "Process Startup");
      if (easySplunkVersion >= 70300)
      {
         $('.dashboard-header > h1').text (gettext("Process Startup"));
      }
      else
      {
         $('.dashboard-header > h2').text (gettext("Process Startup"));
      }
      $('.dashboard-header > p.description').text (gettext("This dashboard displays detailed information about process startup performance."));
   }
