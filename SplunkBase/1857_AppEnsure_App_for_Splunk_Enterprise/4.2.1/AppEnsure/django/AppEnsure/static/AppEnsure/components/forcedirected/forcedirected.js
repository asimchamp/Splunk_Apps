        ...
        formatData: function(data) {
            ...
            data.forEach(function(link) {
                ...
                newLink.source = nodes[sourceName] ||
                        (nodes[sourceName] = {name: sourceName, group: groupName, value: 0,port:link[5]});
                newLink.target = nodes[targetName] ||
                        (nodes[targetName] = {name: targetName, group: link[4], value: 0,port:link[6]});
                newLink.value = +link[3];
                newLink.source.value += newLink.value;
                newLink.target.value += newLink.value;
var srctmp="SOURCE=="+"Name:"+newLink.source.name+"|| groupName:"+newLink.source.group+"|| value:"+newLink.source.value+"|| port:"+newLink.source.port;
var trgtmp="Target=="+"Name:"+newLink.target.name+"|| groupName:"+newLink.target.group+"|| value:"+newLink.target.value+"|| port:"+newLink.target.port;
console.log(srctmp);
console.log(trgtmp);
                links.push(newLink);
            });
        ...
        updateView: function(viz, data) {
            ...
            var txt = graph.selectAll("text.txt")
                    .data(data.nodes)
            ...
            force.nodes(data.nodes)
                    ...
                    .on("tick", function() {
                        link.attr("d", function(d) {
                            ...
                            var offsetX = (diffX * (r * 1.4)) / pathLength;
                            var offsetY = (diffY * (r * 1.4)) / pathLength;
