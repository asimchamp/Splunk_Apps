...
def get_all_internal_inputs(session_key, user, app):
    ...
    response, content = rest_handler.splunkd_request(
        url, session_key=session_key, method="GET"
    )
...
def update_internal_inputs(stanza, data, session_key, user, app):
    ...
    return response["status"] in ["200", "201", 200, 201]
...
def enable_internal_input(stanza, session_key, user, app):
    ...
    response, content = rest_handler.splunkd_request(
        url, session_key=session_key, method="POST"
    )
    return response["status"] in ["200", "201", 200, 201]
