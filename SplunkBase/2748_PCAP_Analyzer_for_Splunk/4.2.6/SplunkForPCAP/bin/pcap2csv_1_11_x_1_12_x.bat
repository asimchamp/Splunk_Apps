...
set "f=%%f"
"%programfiles%\Wireshark\tshark" -o tcp.calculate_timestamps:TRUE -r "!b!\!f!" -T fields -e frame.time -e tcp.stream -e frame.number -e ip.src -e ip.dst -e _ws.col.Protocol -e tcp.srcport -e tcp.dstport -e tcp.len -e tcp.window_size -e tcp.flags.syn -e tcp.flags.ack -e tcp.flags.push -e tcp.flags.fin -e tcp.flags.reset -e ip.ttl -e _ws.col.Info -e tcp.analysis.ack_rtt -e vlan.id -e eth.src -e eth.dst -e http.time -e dns.time -e rpc.time -e smb2.time -e smb.time -e tcp.time_delta -e tcp.time_relative > "%SPLUNK_HOME%\etc\apps\SplunkForPCAP\PCAPcsv\!f!.csv" 
