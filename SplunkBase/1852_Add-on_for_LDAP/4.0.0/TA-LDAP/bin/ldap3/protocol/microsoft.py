...
"""import ctypes""" #replace ctypes with a clunky workaround
...
def dir_sync_control(criticality, object_security, ancestors_first, public_data_only, incremental_values, max_length, cookie):
    ...
    if incremental_values:
        ...
        # Dominique Vocat, avoid using ctypes to not rely on compiled libraries so we flip the bits manually
        flags_bin = bin(flags)

        flags = int(flags_bin, 2)
        if flags_bin[0] == '1': #so if the first bit is a 1 we need to invert due to delivering a signed long
            flags ^= 0xFFFFFFFF
            flags += 1
            flags = -flags
