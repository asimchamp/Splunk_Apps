 ...
 * 
 * Baidu Map module displays a Google Map which is capable of rendering results.
 */
...
var Maps = {
	...
	if(!Maps._scriptAttached) {
		...
		Maps._loaded = true; 
                BMap.Overlay.subclasses = [];
		$(document).trigger('gmaps:loaded'); 
		window._gmapsOnLoad = undefined; 
	    }
	    var s = document.createElement('script');
	    s.src = "//api.map.baidu.com/api?v=2.0&ak=zxpMioGa5YKggNDS6AWGRGb7&callback=_gmapsOnLoad";
	    s.id = 'gmaps';
...
// Asynchronously load the Baidu Map API

Splunk.namespace("Module");
Splunk.Module.BaiduMap = $.klass(Splunk.Module.DispatchingModule, {
    initialize: function($super, container) {
        $super(container);
	this.debug = Splunk.util.normalizeBoolean(this._params.debug);
	if(this.debug) console.debug("BaiduMap module initialized. Attaching onLoad callback");
	this.ct = container;
    ...
    onReady: function() {
	if(this.debug) console.debug("Baidu Map API loaded.");
	var container = this.ct;
	var center = this.getParam("center").split(",");
        var latlng = new BMap.Point(parseFloat(center[0]), parseFloat(center[1]));
	var opts = {
        ...
        var map = new BMap.Map(this.mapCt);
	...
	if(mts) {
		...
		if(!google.maps.MapTypeId[style.toUpperCase()]) {
			...
			Splunk.Messenger.System.getInstance().send("error","splunk","Custom Baidu Map style \"" + style + "\" is not defined!");
		    }
		} else {
		    types.push(style);
		}
	...
	OV = OV();
	    ...
	    if(this.debug) console.debug("Enabling drilldown drilldown_field=%s",this._params['drilldown_field']||"n/a");
            ...
            var me = this;
            $(document).on("drilldown", function() {
                var m = $.makeArray(arguments);
                m.shift();
                me.doDrilldown(m);
            });
        }
	map.addEventListener('idle', this.saveMapState.bind(this));
	map.addEventListener('maptypechange', this.saveMapType.bind(this));
	this.status(false);
	...
	if(this.debug) console.debug("Performing drilldown action for markers %o", markers);
		...
		if(markers.length > 1) {
		    ...
		    // Zoom in
		    var b0 = markers[0].getPosition();
		    var bounds = new BMap.Bounds(b0, b0);
...
Maps.Marker = $.klass({
	...
	return this._pos || ( this._pos = new BMap.Point(this._lng, this._lat));
    },
    ...
    getLatLng: function() {
	return new BMap.Point(this._lng, this._lat);
    }
});
// function _debug() {
...
Maps.OVERLAYS['clusters'] = function() {
    ...
    Maps.MarkerClusterer = $.klass(BMap.Overlay, {
	    ...
	    if(this.markerOpacity) this.markerOpacity = parseFloat(this.markerOpacity);
            ...
            this.map.addEventListener('zoomed', this.onZoomChanged.bind(this));
            this.map.addEventListener('zoomed', this.redraw.bind(this));
            this.map.addEventListener('moveend', this.redraw.bind(this));
        },
        onZoomChanged: function() {
            var maxZoom = this.map.getMapType().getMaxZoom();
        ...
        getExtendedBounds: function(bounds) {
            ...
            var tr = new BMap.Point(bounds.getNorthEast().lat, bounds.getNorthEast().lng);
            var bl = new BMap.Point(bounds.getSouthWest().lat, bounds.getSouthWest().lng);
            var gs = this.gridSize;
            // Convert the points to pixels and the extend out by the grid size.
            var trPix = this.map.pointToOverlayPixel(tr);
            trPix.x += gs;
            trPix.y -= gs;
            var blPix = this.map.pointToOverlayPixel(bl);
            blPix.x -= gs;
            blPix.y += gs;
            // Convert the pixel points back to LatLng
            var ne = this.map.overlayPixelToPoint(trPix);
            var sw = this.map.overlayPixelToPoint(blPix);
        ...
        createClusters: function() {
            ...
            var mapBounds = new BMap.Bounds(this.map.getBounds().getSouthWest(), this.map.getBounds().getNorthEast());
    ...
    Maps.Cluster = $.klass({
        initialize: function(mc) {
            ...
            this.clusterIcon.initialize = this.clusterIcon.onAdd;
            this.map.addOverlay(this.clusterIcon);
        },
        getCenter: function() {
            return this.center;
        },
    ...
    Maps.ClusterIcon = $.klass(BMap.Overlay, {
        ...
        onAdd: function() {
            ...
            this.div.appendTo(panes.markerPane);
            ...
            */
            ...
            this.div.click(function () {
                $.event.trigger('drilldown', c.getMarkers(), self.div.get(0));
            });
            return this.div.get(0);
        },
        getPosFromLatLng: function(latlng) {
            var pos = this.map.pointToOverlayPixel(latlng);
            pos.x -= this.offset.x;
            pos.y -= this.offset.y;
            return pos;
        },
        draw: function() {
            if (this.visible) {
                ...
                this.div.css({ top: pos.y, left: pos.x, position: 'absolute' });
            }
        },
        hide: function() {
            if (this.div) {
        ...
        show: function() {
            if (this.div) {
                this.div.get(0).className = this.getCssClass();
                this.div.show();
                this.updatePos();
            }
            this.visible = true;
