require.config({
    paths: {
        "myAppBase": "../app/GoogleAppsForSplunk",
	"d3" :  "../app/GoogleAppsForSplunk/js/d3.min",
        "bundles" : "../app/GoogleAppsForSplunk/components/bundles/bundles",
	"cubism"  : "../app/GoogleAppsForSplunk/components/cubism/cubism",
	"heatmap" : "../app/GoogleAppsForSplunk/components/heatmap/heatmap",
    },
    shim: {
        "d3": {exports:"d3"},
        "bundles":{ deps:["d3"]},
...
require([
    ...
    _.each($(".heatmap"), function(el){
     require("heatmap",function(HeatMapView){
...
require([
    ...
    _.each($(".cubism"), function(el){
      require("cubism",function(CubismView){
	myRaw = $(el).attr("cubism-options");
    	myOptions = JSON.parse(myRaw);
 	myOptions["el"] = $(myOptions["element"]);	
	myOptions["width"] = $(window).width() - 150;
    	new CubismView(myOptions).render();
      });
    });
});
...
require([
    ...
    _.each($(".bundles"), function(el){
      require("bundles",function(BundleView){
	myRaw = $(el).attr("bundle-options");
    	myOptions = JSON.parse(myRaw);
 	myOptions["el"] = $(myOptions["element"]);	
	myOptions["width"] = $(window).width() - 150;
    	new BundleView(myOptions).render();
      });
    });
});
...
require(['jquery','underscore','splunkjs/mvc'],
	function($, _, mvc){
		...
		$.get("/static/app/GoogleAppsForSplunk/html/README.md", function(data){
