				...
				try:
					...
					if use_multi_threading:
						...
						if num_threads < 0 or num_threads > 10:
							num_threads = 2
						host_asset_params = bool_value(self.qualysConf['setupentity'].get('host_asset_params', '0'))
		...
		try:
			...
			if cp_last_run_datetime:
				...
				if num_threads not in threads_range:
					qlogger.warning("Invalid value %s set for number of threads for Resolve Host Id API. Using default value 2.", num_threads)
					num_threads=2
			
				qlogger.info("PCRS input running in multi-thread mode with %s threads for Resolve Host Id API.", num_threads)

				pcrs_posture_num_threads = int(self.qualysConf['setupentity']['pcrs_posture_num_threads'])
				if pcrs_posture_num_threads not in threads_range:
					qlogger.warning("Invalid value %s set for number of threads for Posture Streaming API. Using default value 2.", pcrs_posture_num_threads)
					pcrs_posture_num_threads=2

				qlogger.info("PCRS input running in multi-thread mode with %s threads for Posture Streaming API.", pcrs_posture_num_threads)
				pcrs_configuration.pcrs_posture_num_threads=pcrs_posture_num_threads
