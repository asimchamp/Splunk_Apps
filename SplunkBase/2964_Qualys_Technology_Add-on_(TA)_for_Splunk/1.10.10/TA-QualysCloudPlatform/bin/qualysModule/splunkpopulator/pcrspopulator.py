        ...
        if api_end_point:
            ...
            try:
				...
				# endif
                    ...
                    self.retrycount_429 += 1
                    self.retry_429= True
                    qlogger.error("Unsuccessful while calling API. Retrying after %s seconds. Error %s. Retry count: %s",retry_interval,err, self.retrycount_429-1)
    ...
    def __fetch_and_parse(self):
        while (self.retrycount <= self.pcrs_max_api_retry_count) or (self.pcrs_max_api_retry_count==0) or self.retry_429== True:
            try:
                ...
                continue
        # endwhile
        if self.retrycount > self.pcrs_max_api_retry_count and self.pcrs_max_api_retry_count != 0 and self.retry_429 == False:
            ids_list=self.ids.split(",",2)
            qlogger.info("Maximum retry limit %s has reached.So skipping the following Policy Id %s with Host Ids %s for endpoint %s ",self.pcrs_max_api_retry_count, ids_list[0], ids_list[2], self.api_end_point)
