...
$IsGC = "False"
$USN = "Unknown"
$MyName = $env:ComputerName + "." + $DomainDNSName
if ($WMI_DOMAIN.Status -eq "OK") {
	...
	if ($MyDC) {
		...
		# Windows Server 2003 - RODC is always false
		#$DN = $entry.Path
		#$ServerEntry = [ADSI]"$DN"
...
if ([string]::Compare($SiteInfoObj.IntersiteTopologyGenerator.Name,$MyName)) {
...
# Windows 2003 does not have DFS-R.
#
$RequiredServices = @( "ntfrs", "netlogon", "kdc", "w32time", "ismserv" )
$srvr = @()
$srvnr = @()
...
# not both.  Since Windows 2003 does not have DFS-R, the count must be 0
$ProcsOK = "False"
if ($srvnr.Count -eq 0){
	$ProcsOK = "True"
}
