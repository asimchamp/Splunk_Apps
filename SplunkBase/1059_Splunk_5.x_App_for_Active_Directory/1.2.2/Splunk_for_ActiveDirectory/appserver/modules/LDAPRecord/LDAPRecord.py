#
# LDAPRecord Python Results Handler - returns JSON to the underlying module
#
# required imports
...
# logging setup
import logging
logger = logging.getLogger('splunk.appserver.controllers.module.LDAPRecord')

class LDAPRecord(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, earliest_time=None, latest_time=None, entity_name='results', postprocess=None):
        ...
        #
        if not sid:
            raise Exception('LDAPRecord.generateResults - sid not passed!')
            
        # get the search jobs
        try:
            job = splunk.search.getJob(sid, sessionKey=cherrypy.session['sessionKey'])
        except splunk.ResourceNotFound:
            return _('<p class="moduleException">[LDAPRecord module] Session Key Not Found</p>')
        ...
        for row in dataset:
            ...
            for f in row.keys():
                ...
                if (f[0] != "_"):
                    ...
                    if (len(v) > 1):
                        rowdata[f] = list();
                        [ rowdata[f].append(x.__str__()) for x in v ]
                    else:
