...
Splunk.Module.Table= $.klass(Sideview.utils.getBaseClass(true), {
    ...
    onMouseover: function(evt) {
        ...
        if (this.getParam("drilldown") == "cell") {
            var cell = (target.is("td")) ? target : $(target.parents().filter("td")[0]);
            // until someone gives a good reason, we don't support drilldown on null cells.
            // and IF someone asks, we can add the functionality behind a param "allowNullDrilldown"
            // for which the default will retroactively be "false" and all will be well.
            if (!cell.text()) {
                cell.addClass("mouseoverHighlight");
            }
        }
    },
    ...
    onMouseout: function(evt) {
        ...
        if (this.getParam("drilldown") == "cell") {
            var cell = (target.is("td")) ? target : $(target.parents().filter("td")[0]);
            cell.removeClass("mouseoverHighlight")
        }
    },
    ...
    getModifiedContext: function() {
        ...
        // there is a selected row.
            ...
            if (this.getParam("drilldown") == "cell") {
                var td = $("td.selected", this.container);
                context.set("click.selectedElement", Sideview.utils.makeUnclonable(td));
                var th = $("th",this.container).eq(td.index());
                var cellField = th.text();
                var name = this.getParam("name");
                context.set(name + ".cell.field", cellField);
                context.set(name + ".cell.value", row[cellField]);
            }
            else {
                var tr = $("tr.selected", this.container);
                context.set("click.selectedElement", Sideview.utils.makeUnclonable(tr));
            }
            context.set("click.selectedTableName", name);
        }
