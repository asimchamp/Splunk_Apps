...
Splunk.Context = $.klass(Splunk.Context, {
    ...
    /* backported from Canary to make action menu customBehaviors work */
    clear: function() {
        this._root = {};
    }
    ...
    Splunk.DashboardManager = $.klass(Splunk.DashboardManager, {
        ...
        initialize: function() {
            ...
            if (Sideview.utils.compareVersions(splunkVersion,"8") > 0) {
                ...
                var methodReference = Splunk.util.redirect_to.bind(Splunk.util);
                Splunk.util.redirect_to = function(someOtherURL, args) {
                    ...
                    function canaryIsNotInstalled() {
                        var url = "/app/sideview_utils/canary_not_installed";
                        var args = {
                            dashboard_name : Sideview.utils.getCurrentView(),
                            hideChrome:"true",
                            hideTitle:"true"
                        }
                        return methodReference(url, args)
                    }
...
/** I will not fear.  Fear is the mind killer. Fear is the little death that brings total
 *  obliteration.  I will face my FieldPicker.      I will permit it to pass over me and through me.
 *  And when it has gone past I will turn the inner eye to see its path.
 *  Where the "setfields" intention was there will be nothing. Only I will remain.
 */
if (Splunk.Module.FieldPicker) {
    Splunk.Module.FieldPicker = $.klass(Splunk.Module.FieldPicker, {
        applyContext: function(context) {
            console.warn("let me explain...  No, there is too much. Nobody could possibly have been using this anyway.")
        }
    });
}
