...
Sideview.utils = {
    ...
    /* backported from Canary in Feb 2021 */
    getTopLevelSearchTokens: function(searchStr) {
        var commands = this.getCommands(searchStr);
        if (commands.length==0) return [];
        searchStr = commands[0];
        var i=0;
        ...
        while (i<searchStr.length) {
            ...
            else if (c == " " && !insideQuotes && parenDepth==0) {
                if (i!=0) {
                    tokens.push($.trim(searchStr.substring(0,i)));
                    searchStr = searchStr.substring(i+1, searchStr.length);
                    i=0;
                    continue;
                }
            }
            else if (i==searchStr.length-1) {
                tokens.push($.trim(searchStr));
                break;
            }
            i++;
