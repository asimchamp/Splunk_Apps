...
def defineOptions(j):
	...
	logger.info("release is " + release)
        if release == "berlin":
                endpoint = url + "u_splunk_incident.do?JSON&sysparm_action=insert"
        elif release == "calgary":
                endpoint = url + "u_splunk_incident.do?JSONv2&sysparm_action=insert"
        elif release == "dublin":
                endpoint = url + "u_splunk_incident.do?JSONv2&sysparm_action=insert"
        elif release == "eureka":
                endpoint = url + "api/now/table/u_splunk_incident"
        else: 
                endpoint = url + "u_splunk_incident.do?JSONv2&sysparm_action=insert"
