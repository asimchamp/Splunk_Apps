...
class ConfigApp(admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        confDict = self.readConf("cisco_Nexus_9k_server_setup")
        if None != confDict:
    ...
    def handleEdit(self, confInfo):
        ...
        try:
            entities = entity.getEntities(['admin', 'passwords'], namespace=APPNAME,
                                            owner='nobody', sessionKey=sessionKey,search="realm=\"" + str(cisco_Nexus_9k_host)+ ',' + APPNAME + "\"")
            if cisco_Nexus_9k_host != xss.unescape(cisco_Nexus_9k_host):
                res = entity.getEntities(['admin', 'passwords'], namespace=APPNAME,
                                            owner='nobody', sessionKey=sessionKey,search="realm=\"" + str(xss.unescape(cisco_Nexus_9k_host))+ ',' + APPNAME + "\"")
                entities.update(res)
        except:
        ...
        if len(list(entities.items())) != 0:
            raise admin.ArgValidationException("Cisco Nexus 9k server credential for " + str(xss.unescape(cisco_Nexus_9k_host)) + " already exists in passwords.conf. Remove it from local/passwords.conf, restart Splunk, and try again.")
        ...
        try:
            ...
            creds["realm"] = str(cisco_Nexus_9k_host) + ',' + APPNAME
