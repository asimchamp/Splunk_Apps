...
var all_totals = {days:[], week:0};
var display_mode = 0, display_modes = [{label: 'minutes', prefix: ''}, {label: '', prefix: ''}, {label: 'kgs CO2', prefix: ''}];
var week_data;
...
function drawEnergyChart(ts, data, aD, mW) {
maxWeeks = mW;
all_data = aD;
console.log("starting drawEnergyChart()");
energy_data = data;
all_totals = calculate_all_weeks(all_data);
base_ts = ts * 1000;
...
var color = d3.scale.linear().domain([0, 1]).range(["white", "red"]);
...
function render_hour_info(d, i) {
	console.log("rendering hour_info");
	...
	switch(display_mode) {
	  case 0:
	    ...
	    d3.select('#status g.second text.value').text(dm.prefix+(kwh*60).toFixed(1));
...
function reset_all_info() {
	console.log("resetting all_info");
        var dm = display_modes[display_mode];
        week_kwh = new Number(all_totals.week);

        d3.select('#status g.first text.time').text(ts_to_datestring(base_ts, 0) + ' - ' + ts_to_datestring(base_ts, 7*maxWeeks+6));
...
function reset_hour_info() {
	console.log("resetting hour_info");
	var dm = display_modes[display_mode];
...
function calculate_totals(week_data) {
	...
	var totals = {days:[0, 0, 0, 0, 0, 0, 0], week:0};
		for(var d=0; d<7; d++) {
			for(var h=0; h<24; h++)
				totals.days[d]+=week_data[d*24+h]*60;
			totals.week += totals.days[d];
		}
	return totals;
}

function calculate_all_weeks (alldata){
	var totals = {days:[0,0,0,0,0,0,0], week:0};
	for (var d=0; d<alldata.length; d++) {
		for(var h=0;h<24; h++)
			totals.days[d%7]+=alldata[d*24+h]*60;
		totals.week += totals.days[d%7];
	}
	return totals;
}
